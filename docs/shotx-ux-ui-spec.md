...
版本：v1.3（对应 PRD v1.5 基线：录屏菜单改版、移除摄像头）  
平台：macOS 14+，菜单栏应用  
需求来源：BRA-25、BRA-36；设计任务：BRA-26、BRA-39；交互修订：BRA-30、BRA-34  
交付对象：macOS 客户端、QA

## 1. 设计边界与使用方式

本文是实现与验收依据；界面名称后的 ID 是开发、埋点错误码和 QA 用例共同引用的唯一标识。若本文与 PRD 冲突，以 PRD v1.5 为准。

- 覆盖全部 30 条 P0 `FR-*`，不包含 P1。
...
工具栏高 40 pt，默认置于选区下方 8 pt；与屏幕边缘、菜单栏或 Dock 冲突时依次翻到上方、选区内底部。若选区容不下工具栏，工具栏留在选区外并在当前显示器可用工作区内贴边，距边缘至少 8 pt；可用宽度不足时拆成“工具 / 输出”两行。工具栏不得进入输出，也不得覆盖尺寸标签或当前拖动把手。

从左到右：选择、箭头、矩形、画笔、文字、马赛克、裁剪｜撤销、重做｜样式控件｜复制（主按钮）｜保存、分享、贴图、关闭。

样式区只展示当前工具适用项：

| 工具 | 可选/移动/删除 | 样式 | 记忆范围 |
| --- | --- | --- | --- |
| 箭头 | 是 | 颜色、1/2/4/8 pt 粗细 | 仅箭头 |
| 矩形 | 是 | 颜色、1/2/4/8 pt 粗细 | 仅矩形 |
| 画笔 | 是 | 颜色、1/2/4/8 pt 粗细 | 仅画笔 |
| 文字 | 是 | 颜色、11/13/16/24/32 pt 字号 | 仅文字 |
| 马赛克 | 是 | 8/16/24/40 px 笔刷 | 仅马赛克 |
| 裁剪 | 否，不生成对象 | 无 | 不记忆 |
...
鼠标命中优先级固定为：已展开菜单/浮层 → 工具栏 → 选区把手 20 × 20 pt 热区 → 标注对象 → 选区内容。无论当前标注工具为何，把手始终可拖动；“选择”工具下拖动选区空白处移动整个选区，其他工具下拖动空白处创建标注，按住 `Space` 拖动则临时移动选区。调整选区时已有标注保持相对屏幕坐标，越出新边界的部分仅在输出时裁切；调整和标注都写入同一撤销栈。选区小于 20 × 20 pt 时隐藏尺寸标签和工具栏但保留八个把手；放大到可容纳后恢复，仍可用键盘调整或按 `Esc` 退出。

初始键盘焦点保留在选区；`Tab/Shift+Tab` 按“选区 → 八个把手 → 工具栏视觉顺序”循环。选区聚焦时方向键移动 1 物理像素、`Shift` + 方向键移动 10；把手聚焦时同样缩放。工具栏或尺寸标签因空间不足隐藏时，从焦点顺序移除；主按钮仅在自身聚焦时响应 `Return`。
...
- 默认按图片物理像素以不超过当前显示器工作区 60% 的尺寸显示；保持宽高比。
- 无标题栏，始终置顶；悬停显示右上关闭、底部透明度滑杆和缩放把手。
- 拖动任意非控件区域移动；拖角缩放，范围 10%–400%；透明度 20%–100%，默认 100%。
- `Esc` 隐藏控件但不关闭；`Cmd+W` 关闭。关闭只释放贴图窗口，不删除原保存文件或当前最近结果。
- VoiceOver 聚焦窗口时先读“贴图，置顶窗口”，然后是关闭、透明度、缩放。
...
| `font-countdown` | SF Pro Display 72/80, bold | 倒计时 |
...
- 工作区宽 < 900 pt：`O-ANN` 可拆成两行（工具/输出）；`O-RSET` 保持 300 pt 面板并截断设备名，选区或工作区不足时滚动正文，底部“返回选区 / 开始录制”固定可见。
...
- 图片标注工具栏在小工作区可拆成两行；控件顺序和能力不变。
- 马赛克笔刷与标注粗细使用本文离散档位，避免开发自行定义连续范围。
...
- PRD v1.5 的 FR 追踪表仍保留摄像头行（`FR-BASE-03` 权限页含摄像头、`FR-BASE-04` 摄像头按需申请、`FR-REC-01` 摄像头控制框、`FR-REC-03` 摄像头预览、`FR-GIF-02` 摄像头叠加），与 v1.5 §9.4/§11.1/§12 的移除口径冲突。本设计按“移除摄像头”口径执行并已在 §16 标记 `FR-REC-03` 为已移除；请产品负责人确认是否同步清理 PRD 表格，并指导 QA 去掉对应验收项。
