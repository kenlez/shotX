...
| FR-CAP-06/07 | `ResultWindow.swift` flattened render, clipboard, PNG and pixelated mosaic (share removed by FR-BRA71-05) | release build | M |
...
| --- | --- | --- |
| FR-BRA71-01 | 马赛克改为矩形对象：`AnnotationView.mosaic(rect:size:)` 填充整块矩形像素化；样式标签「笔刷大小」→「块大小」；仅当两轴均 < 4 pt 时丢弃拖拽，单轴细条保留（BRA-76 B6）；对象就地保留、可选择/拖动/删除/撤销 | `testMosaicDragCreatesRectangularObject`、`testMosaicTinyDragIsDiscarded`、`testMosaicThinStripIsKept`、`testMosaicStyleLabelUsesBlockSize` |
...
| FR-BRA71-03 | 所有对象就地呈现并复用选择/移动/方向键/Delete/历史栈；工具栏恢复「选择」工具入口（BRA-76 B2）；直线/箭头/路径按距离命中 | `testSelectedPathMovesWithDrag`、`testClickFarFromStrokeDoesNotMoveIt` |
| FR-BRA71-04 | 文字改为画布内就地编辑：单击创建 `InlineTextView`（无 `NSAlert`），Return/Esc/点击外部提交，双击文字重入编辑，空文字提交删除，编辑中样式即时反映；重入编辑保留对象原有颜色/字号（BRA-76 B3） | `testTextCommitsInlineWithoutModalAndIsSelectable`、`testDoubleClickReentersTextEditing`、`testDoubleClickReentryPreservesStoredColorAndSize`、`testEmptyTextCommitCreatesNothingAndEmptyEditDeletes`、`testTextObjectDragMovesWithoutEnteringEdit` |
| FR-BRA71-05 | 移除系统分享：`ResultWindow` 分享按钮与 `NSSharingServicePicker` 调用、`CaptureCoordinator` `sharePressed`/`pendingShareImage`/picker delegate 全部删除；菜单栏无分享入口；工具栏输出行为 复制/保存/贴图/关闭；文档/旧清单同步清理（BRA-76 B5） | 静态检查无 `NSSharingServicePicker`/分享引用；`swift test` 通过 |
