...
| 交付基线 | PR #5：分支 `agent/bra-76-fixes` @ `ff7038f`（基 `integrate/bra-55-0.1.2` @ `8cda734`）；PR #4 已关闭并由 PR #5 取代 |
| 包体附件 | BRA-76 重新打包并经 BRA-77 复核（`releases/ShotX-0.1.2.app.zip` / `ShotX-0.1.2-source.zip`） |
...
### 1.2 修复后独立复验（BRA-77，2026-08-13，本仓库）

> 复验对象：PR #5 `agent/bra-76-fixes` @ `ff7038f`。下表全部命令在 `git worktree add <tmp> ff7038f` 的**干净检出**中执行（工作树无任何未提交改动），包体另行解包复核。

| 步骤 | 命令 | 结果 |
| --- | --- | --- |
| 干净检出编译 | `swift build`（调试） | ✅ Build complete（无错误；仅预存 main-actor 闭包警告） |
| 冷缓存单测 | `swift package clean && swift test` | ✅ **57/57 通过**（含 `BRA71AnnotationTests` 19 例 + `SettingsTests` 工具集回退用例） |
| Release 构建 | `make app` | ✅ 成功；`ShotX_ShotX.bundle` 已拷入 `.app/Contents/Resources/`；Info.plist `CFBundleShortVersionString`/`CFBundleVersion` = 0.1.2 |
| 签名校验 | `codesign --verify --deep --strict --verbose=2 ShotX.app` | ✅ `valid on disk`，满足 Designated Requirement |
| 冒烟启动 | `open ShotX.app` + `pgrep -x ShotX` + `osascript` 退出 | ✅ 进程存活并干净退出 |
| 无分享静态检查 | `rg -i "NSSharingServicePicker|sharePressed|shareImage|pendingShareImage" Sources Tests` | ✅ 无残留 |
| 无网络静态检查 | `rg -i "urlsession|urlrequest|nwconnection" Sources` | ✅ 无网络调用（仅 SVG 资源内 xmlns 命名空间） |
| 无模态文字输入 | `rg "NSAlert" Sources` | ✅ 仅存放弃/保存确认框，无文字输入路径 |
| 交付包体复核 | 解包 `releases/ShotX-0.1.2.app.zip` 后 `codesign --verify --deep --strict` | ✅ 交付包 `valid on disk`，满足 DR，版本 0.1.2，含资源 bundle |
| 交付源码包复核 | 解包 `releases/ShotX-0.1.2-source.zip` | ✅ 含 `Sources/ShotX/Assets/Figma/*.svg` 全 15 个（含 `select.svg`） |
| 复验重新打包 | `make package` 后与交付包对比 | ✅ 同尺寸（app 478133 vs 交付 478137，字节级差异仅 zip 元数据；source 72555 一致） |

**B1–B8 逐项独立复核结论（代码层，BRA-77）：**

| # | 结论 | BRA-77 独立证据（干净检出 `ff7038f`） |
| --- | --- | --- |
| B1 | ✅ 关闭 | `Sources/ShotX/Assets/Figma/*.svg`（15 个，含 `select.svg`）与 `Package.swift` `resources: [.process("Assets")]` 均已在提交内；干净检出 `swift build` 直接成功（不再复现 `Bundle.module` 错误）；`make app` 拷入资源 bundle |
| B2 | ✅ 关闭 | 工具栏首按钮为「选择」（`CaptureCoordinator.makeToolbar` 第 1 项 `("select", .select)`，tag 0）；`toolPressed` 按 `AnnotationTool.allCases[index]` 直映射；选择/拖动/Delete/方向键逻辑（`ResultWindow`）存在；`toolChanged` 死代码已删 |
| B3 | ✅ 关闭 | 恢复 300 pt 样式浮层，文字工具含颜色（`makeColorSection`，`tool != .mosaic` 即含文字）与字号滑杆（`BrushSlider`，11–32 pt）；`applyStyleLive` 直接更新活动 `textEditor` 字体/颜色（ResultWindow.swift:295-303）即时反映；双击重入 `beginTextEditing` 取对象存储样式（ResultWindow.swift:497-502）；`testDoubleClickReentryPreservesStoredColorAndSize` 覆盖 |
| B4 | ✅ 关闭 | `makeMemoryRow`/`makeColorSection`/`makeSizeSection`/`BrushSlider`/取色器全套恢复；`presetColors` 12 色（`CaptureCoordinator.swift:1135`）；马赛克标签「块大小」（`AppModel.swift:98`，range 8–40 px，presets 8/16/24/40）；`testMosaicStyleLabelUsesBlockSize` 覆盖 |
| B5 | ✅ 关闭 | `QA-ACCEPTANCE-MATRIX.md`、`RELEASE-NOTES.md`、BRA-38/55 清单「分享」残留已清理（仅保留「分享已移除（FR-BRA71-05）」说明）；代码/测试无 `NSSharingServicePicker` 等 |
| B6 | ✅ 关闭 | 丢弃条件改 `shape.width < 4 && shape.height < 4`（ResultWindow.swift:261）；新增 `testMosaicThinStripIsKept`（3×50 细条保留）；`testMosaicTinyDragIsDiscarded` 仍通过 |
| B7 | ✅ 关闭 | 迷你面板 `sizeDotPressed`/`textStylePressed`/迷你 `makeOptionsContent` 已删，插值错误随代码移除消除；恢复面板使用规范插值（`"\\(tool.styleLabel) \\(Int(value)) \\(range.unit)"`） |
| B8 | ✅ 关闭 | `AnnotationTool` 恢复 7 工具集 `[.select, .arrow, .rectangle, .pen, .text, .mosaic, .crop]`（无 `.ellipse`/`.line`）；选区边框回 `controlAccentColor`；12 预设色；无暗色工具栏重绘；`SettingsTests.testAnnotationToolSetMatchesOriginalFigmaTools` 覆盖 |

**分 FR 结论（BRA-77 独立）：**

- **FR-BRA71-01** — 通过（逻辑层）。矩形马赛克对象 + 块大小 + 就地保留 + `&&` 丢弃条件均有代码与测试证据；「导出后不可恢复底图」属渲染/输出性质，需真实桌面实测（§2.1 TC-05）。
- **FR-BRA71-02** — 通过（逻辑层）。逐事件采样 + Catmull-Rom→Bezier、<2pt 点笔、距离命中均有代码与测试证据；手感需人工实测。
- **FR-BRA71-03** — 通过（逻辑层）。「选择」工具入口已恢复（B2 关闭），选择/拖动/Delete/方向键/撤销逻辑存在并有测试覆盖；真实交互手感需人工实测。
- **FR-BRA71-04** — 通过（逻辑层）。就地编辑 + 无模态 + 双击重入保留样式 + 浮层颜色/字号即时反映（B3 关闭）均有代码与测试证据；编辑手感需人工实测。
- **FR-BRA71-05** — 通过（代码层）。分享 UI/API/测试残留为零（B5 关闭）；复制/保存/贴图保留。
- **FR-BRA71-06** — 通过（逻辑层）。`RecordingSetupLayout.frame` 中心锚定 + 小选区避让链 + 贴边回退逻辑与 4 例单测对齐；开始录制 `setup?.close()` + `SCContentFilter` 排除本应用窗口，成片排除逻辑成立；中心跟随/避让/成片需真实桌面实测。
...
> ✅ **B2 已修复（BRA-76，BRA-77 复核关闭）**：工具栏首按钮恢复「选择」入口（`select.svg`，tag 0 → `.select`），绘制工具后可切回选择态。下述用例现可执行。
...
> ✅ **B3 已修复（BRA-76，BRA-77 复核关闭）**：恢复 300 pt 样式浮层，文字工具含颜色与字号滑杆并在画布内即时反映（`applyStyleLive` 更新活动输入框）；双击重入保留对象原色/字号。TC-BRA71-04-04 现可执行，颜色即时反馈可测。
...
> 2026-08-13 BRA-77 独立复验：干净检出 `ff7038f` 全量构建/测试/包体复核通过，B1–B8 均在代码层确认关闭（证据见 §1.2）。**当前无阻断项；剩余均为真实桌面手测项。**
...
- **B8（范围外）**：`.ellipse`/`.line` 工具、选区边框重绘、12→6 色、暗色工具栏均非 PRD §1.1 内容，需产品确认是否保留。 → **BRA-76 已修复**：撤回 `.ellipse`/`.line` 与视觉重绘，恢复原有工具集、`controlAccentColor` 边框与 12 色。 → **BRA-77 复核**：✅ 工具集与视觉均已恢复（§1.2）。

## 4. 反馈字段

- 通过数 / 总数：**后台逻辑 30/30 通过**（57/57 单测 + 静态检查 + 干净检出构建/打包/签名）；§2 手测 34 例均**需人工实测**（马赛克渲染不可逆、画笔手感、文字就地编辑手感、录屏菜单中心跟随/避让与成片排除——见 §2 各 TC；后台可自动验证部分见 §1/§1.0/§1.2）
- 未通过用例（TC ID、期望、实际、截图/录屏）：
  - 无（B1–B8 阻断项已全部关闭；无已知未通过用例）
- 无法测试及原因：全部 §2 手测用例依赖真实桌面交互，本环境无法执行；未执行的手测一律不标记「通过」
- 其他问题与建议：B1–B8 已关闭（BRA-77 独立复核证据 §1.2）；剩余工作为真实桌面手测（§2）与 PR #5 合入决策；BRA-78/79 的 Figma 录屏 UI（FR-BRA71-07）为独立范围，不属本清单
